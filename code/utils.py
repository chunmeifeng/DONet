...
def AdaptiveCoilCombine(img, Rn=None):
    """
    ...
    Rs = np.zeros((H, W, coils, coils)).astype(np.complex128)
    ...
    N = 2
    for i in range(N):
        v = np.squeeze(np.sum(Rs * np.transpose(np.tile(v,(coils,1,1,1)), (1,2,3,0)), axis=2))
