...
def getModel(X, X_k, mask, alpha):
    ...
    elif args.name.find('dense')>=0:
        with open('../experiment/{}/with dense indeed.txt'.format(args.name), 'w') as f:
            f.write('Modle is with dense indeed.')
        C = args.n_feats // 2
        temp = X
        for i in range(args.n_blocks):
            conv1_h, conv1_l = firstOct_complex_conv2d(temp, 'conv' + str(i + 1) + '1', kw=3, kh=3, n_out=C, sw=1, sh=1, activation=True, alpha=alpha)
            
            conv2_h, conv2_l = oct_complex_conv2d([conv1_h,conv1_l], 'conv' + str(i + 1) + '2', kw=3, kh=3, n_out=C, sw=1, sh=1, activation=True, alpha=alpha, with_dem=False)
            
            conv3_h, conv3_l = oct_complex_conv2d([tf.concat([conv1_h, conv2_h], axis=-1), tf.concat([conv1_l, conv2_l], axis=-1)], 'conv' + str(i + 1) + '3', kw=3, kh=3, n_out=C, sw=1, sh=1, activation=True, alpha=alpha, with_dem=False)
            
            conv4_h, conv4_l = oct_complex_conv2d([tf.concat([conv1_h, conv2_h, conv3_h], axis=-1), tf.concat([conv1_l, conv2_l, conv3_l], axis=-1)], 'conv' + str(i + 1) + '4', kw=3, kh=3, n_out=C, sw=1, sh=1, activation=True, alpha=alpha, with_dem=False)
            
            conv4_h = tf.concat([conv1_h, conv2_h, conv3_h, conv4_h], axis=-1)
            conv4_l = tf.concat([conv1_l, conv2_l, conv3_l, conv4_l], axis=-1)

            conv4_h = tf.layers.conv2d(conv4_h, int(conv4_h.get_shape()[-1])//4, (1,1), padding='SAME', activation=None, use_bias=True)
            conv4_l = tf.layers.conv2d(conv4_l, int(conv4_l.get_shape()[-1])//4, (1,1), padding='SAME', activation=None, use_bias=True)
...
if __name__ == "__main__":
    ...
    args.name = 'Dual-Oct_dense_B10_a0.125_cpd320_1Un3'
    args.alpha = 0.125
    ...
    y = getModel(X,X_k,mask,alpha=0.125)
