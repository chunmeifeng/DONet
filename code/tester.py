...
def test(test_data, mask):
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        if args.rsname is not None:
            ...
            best_epoch = int(tf.train.latest_checkpoint(from_model_dir).split('-')[-1])//int(np.ceil(args.nb_train//args.batch_size))
        ...
        if args.test_only:
            ...
            for y_test, n_batch in iterate_minibatch(test_data, batch_size=4, shuffle=False):
                ...
                for i in range(batch_prediction.shape[0]):
                    ...
                    if args.in_channels == 2:
                        label_acc = np.squeeze(real2complex_array(label))
                        prediction_acc = np.squeeze(real2complex_array(prediction))
                    else:
                        label_acc = AdaptiveCoilCombine(real2complex_array(label))
                        prediction_acc = AdaptiveCoilCombine(real2complex_array(prediction))
            ...
            print('[Epoch{:4d}]  PSNR: {:.4f} (Best:{:.4f} @epoch {})  SSIM: {:.4f} (Best:{:.4f} @epoch {})'.format(
